...
void VulkanEngine::init_swapchain()
{
	...
	//hardcoding depth format to 32 bit float
	_depthFormat = VK_FORMAT_D32_SFLOAT;

	//depth image will be an image with the format we selected and depth attachment usage flag
...
void VulkanEngine::init_pipelines() {
	...
	//we don't use multisampling, so just run default one
	...
	//make sure triangleFragShader is holding the compiled colored_triangle.frag
	pipelineBuilder._shaderStages.push_back(
		vkinit::pipeline_shader_stage_create_info(VK_SHADER_STAGE_FRAGMENT_BIT, triangleFragShader));

	//start from just default empty pipeline layout info
	...
	// takes up size of a MeshPushConstants struct
	push_constant.size = sizeof(MeshPushConstants);
	//accessible only in vertex shader
...
bool VulkanEngine::load_shader_module(const char* filePath, VkShaderModule* outShaderModule) {
	...
	// because cursor is at end, it gives size directly in bytes
	size_t fileSize = (size_t)file.tellg();

	// spirv expects buffer to be on uint32, so make sure to reserve an int
	...
	// check that the creaion goes well
...
VkPipeline PipelineBuilder::build_pipeline(VkDevice device, VkRenderPass pass) {
	...
	//setup dummy color blending. Just setting blending to "no blend"
	...
	//It's easy to error out on create graphics pipeline, so we handle
	// it a bit better than the common VK_CHECK
...
{
	//make model view matrix for rendering the object
	//camera view
	...
	for (int i = 0; i < count; i++)
	{
		...
		//final render matrix, that we are calculating on CPU
		...
		//only bind mesh if it's a different one from last bind
